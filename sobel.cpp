...
void sobel(cv::Mat &image, cv::Mat &mag, cv::Mat &dir, int sobelSize)
{
    ...
    cv::Sobel(image, dx, CV_64F, 1, 0, sobelSize);
    cv::Sobel(image, dy, CV_64F, 0, 1, sobelSize);
    ...
    for (int row = 0; row < image.rows; ++row)
    {
        for (int col = 0; col < image.cols; ++col)
        {
            ...
            scaled_mag_val = static_cast<uint8_t>(lerp(deLerp(mag_val, mag_min, mag_max), 0, std::numeric_limits<uint8_t>::max()));
        }
    }
}
